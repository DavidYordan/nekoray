...
CTest 当前包含 1 项 `config_recovery_test`，验证覆盖备份、隔离证据与删除前快照 helper；这不代表 ProfileManager 多文件事务、ConfigBuilder、导入、GUI、TUN/WFP 或系统代理已有 C++ 自动验收。发布前还必须完成 [测试矩阵](../testing/TEST_MATRIX.md)。

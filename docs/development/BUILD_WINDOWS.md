...
`-ReferenceDir` 默认留空，只允许用户显式提供一个经过核对的 geodata 缓存目录；脚本会按规范化直接路径拒绝 `D:\Program Files\nekoray` 及其子目录，并逐组件拒绝 `~` 与 ReparsePoint/junction。当前检查没有通过最终文件句柄消解 SUBST、设备路径等其它别名，操作者还必须独立确认 reference 根的最终目标；也不得把任何其它安装目录的 core、配置或进程当作构建输入。
...
CTest 当前包含 1 项 `config_recovery_test`，验证覆盖备份、隔离证据、删除前快照、事务提交/模拟回滚/锁/pending、before/after 恢复与方向锁、单文件 `VerifiedBefore`/`VerifiedAfter`/`Indeterminate` intent、终态退役，以及 hidden/unexpected/exact-case/staging。它还验证合法 terminal schema/id/state header 即使 entries 损坏/为空也不阻断 startup，而 report 必须标记 `valid=false`；非法 terminal schema 仍阻断。这不代表订阅和非空 group 已完成事务化，也不代表 ConfigBuilder、导入、GUI、TUN/WFP 或系统代理已有 C++ 自动验收。发布前还必须完成 [测试矩阵](../testing/TEST_MATRIX.md)。

...
截至 2026-07-22，当前源码已完成一次不带 `-SkipGoBuild`/`-SkipGuiBuild` 的本地完整打包；`build-package-windows64/` 与 `deployment/windows64/` 的 GUI/core hash 一致，zip 也已生成，212 个 package 配置文件已恢复且无 preserve 目录遗留。这些 deployment/zip 是被忽略的本地验收产物；干净工具链、版本化 release manifest 与 Windows 集成矩阵未完成前仍不得作为候选交付物。

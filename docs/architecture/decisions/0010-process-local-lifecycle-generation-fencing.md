...
修订说明：本 ADR 的 GUI/core 串行化基础继续有效；跨 daemon 身份、ready、服务端 deadline、超时对账与 lifecycle v3 Exit 闭环已由 [ADR 0011](0011-daemon-identity-and-lifecycle-reconciliation.md) 收紧。下文已按现行实现更新，不得再使用本地 generation 上界替代 RPC receiver 身份。
...
进程内 Start/Stop、Exit 和 core instance 的直接竞态得到明确 fence，运行状态可附带 generation 与 config hash 供诊断；代价是 blocked、ACK 不确定或进程不退出时会选择持续阻断而不是自动 fallback/kill/replacement。下一步仍按 ADR 0008 建立持久 RuntimeStateMachine、Windows Service、stable anchor 和 persistent WFP，并把当前 context-aware executor/generation 机制迁移为可恢复的受保护 OS 事务。

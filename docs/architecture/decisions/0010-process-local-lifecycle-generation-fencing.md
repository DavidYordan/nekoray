...
状态：部分有效；仅保留已证明的进程内竞态修复，persistent Runtime/WFP 推论已被产品契约取代
日期：2026-07-22

修订说明：本 ADR 的 GUI/core 串行化基础继续有效；跨 daemon 身份、ready、服务端 deadline、超时对账与 lifecycle v3 Exit 闭环已由 [ADR 0011](0011-daemon-identity-and-lifecycle-reconciliation.md) 收紧。2026-07-27 后，[产品契约](../../PRODUCT.md)已明确 persistent Runtime/WFP 不是核心需求或发布前提；本文中由它推导出的全局 TUN/退出阻止和后续路线不再有效。
...
这些问题会让 GUI 的 running/TUN 显示、core 中真实 generation 和迟到异步完成互相脱节，因此需要局部止损。它们不授权继续实现 [ADR 0008](0008-persistent-windows-runtime.md) 的历史候选架构。
...
5. 当前 core 崩溃后只重启**空控制 core**，不自动恢复 profile、TUN 或辅助线路；数据面会消失。该行为必须与 `adef6cd` 和真实稳定性风险对照，不能仅凭旧 persistent WFP 前提宣布为发布阻断，也不能据此自动恢复用户网络模式。
...
10. gRPC `Call` 使用 `QSemaphore` 建立跨线程完成同步。每个 `Client` 为 Start/Stop/Exit/Reconcile 生成单调 command sequence；Go lifecycle 在同一 single-owner executor 内推进 watermark。`ReconcileLifecycle` 的更高序号会推进 watermark，但不会覆盖最后一条 lifecycle command 记录：目标先取得 executor 则 barrier 等待其终态，barrier 先取得则迟到目标因 stale sequence 被拒绝。它不是只读状态查询。协议 v3 为有界调用发送标准 `grpc-timeout`，服务端 deadline 比本地 abort 提前 250 ms；等待准入的命令可无状态变更退出，Start cancellation 与 runtime publication 只允许一方获胜。已准入 Stop/Close 不可安全中断，unknown 仍不能盲目重试。

### Go core

1. 单一 lifecycle command executor 串行化 candidate 创建/发布、Stop、dial、stats 和 Exit 状态检查，并允许尚未准入的调用响应 context。Start/Stop/Exit 在 phase 检查前验证 command sequence、推进 session-local ordering watermark，并记录各自 lifecycle outcome；零值、重复或低于已接受序号的命令 fail closed。candidate 只有完整启动成功且 publication 赢得取消竞态后才发布。
...
- 服务端 deadline、准入取消和 Start 发布仲裁已经建立，但它们不是通用可回滚事务：Stop 一旦准入并开始 Close 就不能安全中断，对账自身再次超时后 GUI 仍必须保持 indeterminate。
...
因此现有 TUN reload/退出 guard 不能由本 ADR 自动证明合理，必须按上游行为和现行产品条目逐项审计；也不得把本 ADR 用作真实 Windows TUN/WFP、OS 资源退出或无泄漏切线验收证据。项目当前不可发布的原因见[接管状态](../../TAKEOVER_STATUS.md)，不是“尚未实现 persistent WFP”这一单项。
...
- Go lifecycle/auth/gRPC 测试覆盖失败/取消 candidate、blocked、旧 reference、命令排序、deadline 前置准入 fence、Exit 的 STOPPED 前置条件/`EXITING` 终态、barrier 先于/后于 Start/Stop/Exit、精确 target/hash/outcome、token + UUID、协议 v3 和真实 localhost gRPC ACK 后 GracefulStop。
...
进程内 Start/Stop、Exit 和 core instance 的直接竞态得到明确 fence，运行状态可附带 generation 与 config hash 供诊断；代价是 blocked、ACK 不确定或进程不退出时会选择持续阻断而不是自动 fallback/kill/replacement。后续只按已复现竞态维护或简化这些机制；现行下一步见[已知问题与整改安排](../../KNOWN_ISSUES.md)，不得继续按 ADR 0008 扩建。

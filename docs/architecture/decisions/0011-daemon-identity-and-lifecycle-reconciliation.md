...
GUI 会在同一 localhost 端口、token 和 HTTP/2 client 上重建 `nekobox_core`。本地 QProcess generation 只能描述 GUI 看到的进程，不能证明某次 RPC 实际落在哪一代 daemon。旧实现又把日志中的 `grpc server listening` 当作 ready；该日志实际来自 stderr，监听端口也可能属于旧进程。Start/Stop 响应丢失后，普通状态快照还会与尚未取得 lifecycle executor 的迟到 handler 竞争，不能安全证明“命令未执行”。
...
5. Start/Stop 各自记录 sequence、kind、服务端按收到的精确配置字节计算的 SHA-256 及终态 outcome。`ReconcileLifecycle` 自身取得更高 command sequence，并与 Start/Stop/Exit 共用同一 lifecycle executor：
...
8. lifecycle protocol v3 加入标准服务端 deadline、可取消准入与 Start cancellation-vs-publication 仲裁，并保留结构化 Exit lifecycle snapshot。Go 仅在精确 `STOPPED` 时原子提交 `EXITING`，ACK 必须回显同一 daemon UUID、command sequence/watermark、空 current/config、`EXIT/SUCCEEDED`；active/blocked 不会被隐式 Stop。响应形成后通过 handler 外 one-shot callback 调用 gRPC `GracefulStop`，避免同步停止当前 handler 的死锁。
...
- 同 executor 的对账只能返回稳定终态或超时 unknown，不是 STARTING/STOPPING 实时进度 API。
...
- gRPC 测试覆盖握手回显、协议版本 3、deadline 准入 fence、Start 取消/发布、Start/Stop/Exit 对账响应和非递增屏障拒绝。

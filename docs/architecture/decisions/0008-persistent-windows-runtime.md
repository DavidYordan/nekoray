...
状态：Superseded / 未获产品授权
日期：2026-07-20
最后复核：2026-07-22

> 2026-07-27：该方案不再是核心需求或发布前提，只保留为历史研究材料；未经用户另行确认不得继续实施。
...
- Go gRPC 的数据面变更只有 `Start`/`Stop`，已有 instance 时拒绝再次启动；`GetDaemonInfo` 只做精确实例握手，`ReconcileLifecycle` 则以更高 sequence 取得 lifecycle executor、推进 ordering watermark 并返回进程内终态。后者不是只读查询，两者也都不是 Windows OS 事实源；系统仍没有 `Prepare`、`Commit`、`Rollback` 或原地 `Reload` RPC。
...
- 普通 GUI 的 core RPC 仅监听 localhost，并用每个 GUI session 随机令牌鉴权；同一会话内 daemon 重启沿用 token。每次启动另有 UUID，它只充当实例 fence、不是凭据；所有 RPC 都在 handler 前验证这个精确身份。Start/Stop/Exit 使用 session-local command sequence；Start/Stop 响应不确定时，同 executor 的更高序号 `ReconcileLifecycle` 会推进 watermark，并可返回 target outcome/config hash/phase。返回的 `ACTIVE`/`STOPPED` 只描述该 daemon 的内存状态，不证明 Windows 资源。lifecycle protocol v3 加入服务端 deadline 和 Start cancellation-vs-publication 仲裁，并把 Exit 收敛为精确 `STOPPED → EXITING` 的结构化 ACK、handler 外 `GracefulStop` 和 GUI 的 `{generation, UUID, PID}` QProcess-finished wait；active/blocked 不隐式 Stop，ACK 不确定时只有 exact non-admission 对账可恢复，其余持续阻断且不 kill/replacement。该闭环仍只属于 GUI-owned daemon，不证明 Windows 资源。未准入命令能按 context 取消，但已准入 Stop/Close 不可安全中断；对账再次超时仍为 unknown，`GracefulStop` 也可能等待在途 handler，因此尚无通用、持久化的端到端事务。`nekobox_core run/check` 是另一个由用户显式调用、测试工具依赖的高级入口。Go 层尚未重复 C++ ConfigBuilder 的产品策略校验，未来 Runtime 必须统一配置授权、generation 和执行入口，但不能把这一缺口误写成普通 GUI 的任意旁路。

...
- 普通 GUI 的 core RPC 仅监听 localhost，并用每个 GUI session 随机令牌鉴权；同一会话内 daemon 重启沿用 token。每次启动另有 UUID，它只充当实例 fence、不是凭据；所有 RPC 都在 handler 前验证这个精确身份。Start/Stop/Exit 使用 session-local command sequence；Start/Stop 响应不确定时，同 mutex 的更高序号 `ReconcileLifecycle` 会推进 watermark，并可返回 target outcome/config hash/phase。返回的 `ACTIVE`/`STOPPED` 只描述该 daemon 的内存状态，不证明 Windows 资源。30 秒 client abort 与 5 秒对账仍只界定 GUI 等待，Go handler/屏障不保证按 context 中止，对账再次超时仍为 unknown，因此尚无端到端 deadline；Exit 也仍缺可靠 ACK 与精确 QProcess-finished 闭环。`nekobox_core run/check` 是另一个由用户显式调用、测试工具依赖的高级入口。Go 层尚未重复 C++ ConfigBuilder 的产品策略校验，未来 Runtime 必须统一配置授权、generation 和执行入口，但不能把这一缺口误写成普通 GUI 的任意旁路。

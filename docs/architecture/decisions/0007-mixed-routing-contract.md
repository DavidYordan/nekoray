...
主入口与至少两个辅助端口分别连接可观察的不同 mock/真实出口。对辅助端口 A 注入故障时，A 的请求必须失败且不能到达主线、辅助 B 或 direct；同时发往主入口和 B 的独立请求应继续按各自语义工作。负向测试覆盖 custom 覆盖、空链、端口冲突、DNS 策略、front proxy 与候选启动失败；新增候选失败不得破坏既有 listener，已持久化端口冲突不得换号。这里不要求 persistent Runtime、WFP 或自动回滚。

...
当前 GUI 同时拥有主 core和网络数据面生命周期。数据面变更仍只有整 Box 的 Start/Stop，没有 Reload/Prepare/Commit；Stop关闭内部 TUN、Mixed、DNS、outbounds、Wintun和动态 WFP session。`GetDaemonInfo` 与 `ReconcileLifecycle` 只提供进程身份/状态对账，不改变这一数据面限制。它意味着切线/重载会重建整 Box，但不自动产生 persistent WFP 或无缝 TUN 切换的产品要求；相关额外阻止必须逐项对照上游。无 instance 时的内部 proxy helper system fallback 已封死，Start/Stop/dial/stats/Exit 也已由 process-local single-owner command executor 和 generation-bound reference 串行。

普通 GUI 只连接 core 的 localhost gRPC，token 每个 GUI session 随机生成并经 stdin 传给 core；同一 GUI 会话内 daemon 重启沿用该 token，没有 token 仍不能调用 RPC。每次 QProcess 启动另生成不可变 UUID，所有 RPC 在 handler 前验证该精确身份；日志只触发 `GetDaemonInfo` 身份/协议 v3 握手，不能独自构成 ready。`Start`/`Stop`/`Exit` 另带单调 command sequence；Go lifecycle 在同一可感知 context 的 single-owner executor 内记录最高序号。GUI 为 RPC 发送标准 `grpc-timeout`，且服务端 deadline 早于本地 abort 250 ms：等待准入的命令可在超时后不推进 watermark；已准入的 Start 由 cancellation-vs-publication 原子仲裁，取消获胜会清理 candidate，发布获胜则视为已提交并交由对账。Stop 一旦准入并开始 sing-box shutdown，当前 Close API 仍不可安全中断。响应不确定时，更高序号 `ReconcileLifecycle` barrier 会等待目标终态或先封住迟到目标，并返回 target outcome、config hash 与当前 phase。Exit 只允许精确 `STOPPED` 原子进入 `EXITING`，先返回结构化 ACK，再由 one-shot `GracefulStop` 结束 server；active/blocked 不隐式 Stop。GUI 冻结 `{generation, UUID, PID}` 并持续等同一 QProcess `NormalExit/0`，不 kill/replacement。ACK 丢失时，只有更高序号对账精确证明 `STOPPED/FENCED_NOT_ADMITTED` 才恢复控制，否则保持 continuation fence 并继续等 finished。对账自身再次超时、已准入 Stop 或 `GracefulStop` 等待在途 handler 时仍可能 unknown，所以这仍不是完整、持久化的端到端事务。`nekobox_core run/check` 是用户显式执行的高级 CLI，构建和隔离测试工具依赖它直接读取 raw config；它不经过 C++ ConfigBuilder policy 属于工具边界事实。只有产品启动链可绕过或 CLI 被另行要求执行同一策略时，才需要在 Go 侧增加相应窄校验，不能仅为“分层一致”复制整套 GUI policy。

当前安全隔离会拒绝任何 inbound `set_system_proxy=true` 和未由产品 TUN 开关生成的 TUN；受管 TUN 必须与完整生成对象一致，并保留生成的接口自动检测、不得出现 default/bind-interface 覆盖。系统 WireGuard/Tailscale endpoint 与 NTP 写系统时钟也被拒绝，`internal-full` 则与产品 TUN、辅助并发和测试路径隔离。顶层 custom merge 另以完整 listener/outbound 快照保护受管 Mixed。专用辅助入口的规则顺序为 sniff、仅匹配该 inbound 的显式 reject/block、精确 chain terminal；普通 redirect/direct/bypass 规则位于 terminal 之后。上述均是配置时窄 guard，不代表任意路由已证明安全，也没有改变 GUI 退出会带走 worker 的架构事实。
...
GUI TransitionCoordinator（Start/Stop/CrashCleanup 单一 ticket）
  ...
  -> gRPC v3（token + UUID；server deadline / monotonic command sequence / reconcile barrier / Exit ACK）
  -> Go coreLifecycle single-owner executor

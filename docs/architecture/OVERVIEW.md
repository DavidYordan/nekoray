...
当前 GUI 同时拥有主 core和网络数据面生命周期。数据面变更仍只有整 Box 的 Start/Stop，没有 Reload/Prepare/Commit；Stop关闭内部 TUN、Mixed、DNS、outbounds、Wintun和动态 WFP session。`GetDaemonInfo` 与 `ReconcileLifecycle` 只提供进程身份/状态对账，不改变这一数据面限制。这一结构不能满足 TUN下 fail-closed切线。无 instance 时的 system TCP/UDP/HTTP helper fallback 已封死，Start/Stop/dial/stats/Exit 也已由 process-local single-owner command executor 和 generation-bound reference 串行，但持久保护层尚不存在。

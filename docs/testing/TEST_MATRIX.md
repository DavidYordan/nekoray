...
| 项目 | 结果 | 证明范围 |
|---|---:|---|
| 当前源码 Windows 全量 C++/package 本地重编译 | 成功 | 无 Skip 流程先以受保护 helper 清空并重建 GUI build tree，再编译 lifecycle v2 Exit、QProcess finished tracker 和 raw integration harness；没有继承旧 CMake cache、object 或手工诊断 core。只证明本机工具链的 clean provenance，不是独立 clean-room release |
| 两个 Go 模块普通测试 | 通过 | core 包含 nil-config/system-fallback/FullTest、lifecycle generation、Exit `EXITING`/对账与真实 localhost gRPC ACK→GracefulStop；`grpc_server` 覆盖 token + UUID、协议版本 2、one-shot shutdown controller 与 metadata 边界，不是 GUI/Windows 资源集成测试 |
...
| CTest | 2/2 | 在项目 MinGW `bin` 已加入 `PATH` 的环境中通过。`runtime_transition` 除原有 transition/queue/crash 竞态外，覆盖身份快照和 `{generation, UUID, PID}` finished tracker 的精确身份、异常完成、重复信号及 finished-before/after-wait。CTest 不创建 QProcess/GUI/core，也不执行真实 HTTP/2；这些纯测试不代表订阅/非空 group、完整 ConfigBuilder snapshot 或 Windows TUN/WFP 已验收 |
| raw core Exit integration | 完整无 Skip package gate PASS | 用随机 loopback control port、无 listener/无 TUN 配置和刚构建 core，验证 lifecycle v2 握手、错误 UUID 为 gRPC 16、Exit non-admission 对账 fence/迟到命令拒绝、active Exit 为 gRPC 9、显式 Stop、结构化 `EXITING` ACK、同一 QProcess `NormalExit/0` 和常见 WinINet 五键不变。它不调用产品 `Client::Exit`/MainWindow，不验证生产 PID/`2080`、适配器、路由、DNS、TUN 或 WFP |
| Windows quality CI | 通过 | 仓库卫生、固定子模块、受控 core 源构建、Go 普通测试和 verifier 安全契约；不覆盖 GUI/TUN/WFP |

本轮测试使用受保护 clean reset 后重建的 GUI 和 package core：`build-package-windows64/nekobox.exe` 与 `deployment/windows64/nekobox.exe` 的 SHA-256 均为 `DC9DFBEFB06160FBB559B5579C00C2BCFAF625C79AB66486176F78FFB5728384`；core 仅存在于 `deployment/windows64/nekobox_core.exe`，SHA-256 为 `ADE5B5EC46CE67E0F9D324B7F543AC82032C0071074B09F9056B90D2E865E59D`，不声称 build tree 中存在第二份 core。不带 Skip 参数的完整打包及 tracker/raw Exit gate 已成功，zip SHA-256 为 `6471D4F578A7CFFD0587B5671E38000CC29868148A2BA034A57854B812FF7117`，package RouteFluent manifest SHA-256 为 `028DF56E733869CCFE150544F65D2FA8469795325C17C94ECCAF8E5091AA7C22`。212 个 package 配置文件已恢复，没有 preserve 或手工诊断产物残留。这些 hash 只是被忽略的本地审计快照，不是 release manifest。以上均不是 Windows TUN/WFP、系统代理 broker、GUI→Client 退出或线路切换验收。
...
| L3 Windows 集成 | 生命周期 | GUI 退出/重启、线路重启、core 崩溃与显式回滚；RPC identity/端到端 deadline/indeterminate 对账 | 每次 core 启动分配 UUID，全 RPC 在 handler 前验证身份，并以协议 v2 `GetDaemonInfo` 精确回显后才 ready。Start/Stop 使用同 mutex 的高序号 `ReconcileLifecycle` barrier；返回的 active/stopped 仅为 daemon 内存态。Exit 只从 `STOPPED` 进入 `EXITING`，返回结构化 ACK 后 `GracefulStop`；GUI 冻结 generation/UUID/PID、持续等同一 QProcess `NormalExit/0`，不 kill/replacement，ACK 不确定时只有 exact non-admission 对账才恢复。raw core gate覆盖真实 QProcess/HTTP2 的正常 Exit，但不调用产品 Client/MainWindow，且无 listener/TUN。30 秒 client abort/对账仍不能取消服务端操作，`GracefulStop` 也可等待在途 handler；缺 GUI→Client、ACK 丢失、父进程死亡和 Windows OS 资源验收 | 阻断发布 |
...
| L1/L3 | C++/Go/脚本自动测试 | CTest、自有 Go 模块与隔离导出 fixture | CTest 保持 2 项纯测试；tracker 和 Go 单测覆盖 Exit v2/对账/finished 顺序，完整 package 另有真实 core raw QProcess/HTTP2 gate。尚无 GUI→Client crash→commit、HTTP/2 timeout/ACK 丢失、TrafficData 并发或 ProfileManager/订阅完整 harness。两个 Go 模块普通测试通过，core module 的重复/race/vet 通过；PowerShell/Python fixture 有独立证据，但 ConfigBuilder/import 和持久 runtime 仍缺 C++/Windows 集成矩阵 | 阻断稳定版 |
...
- `test/core_exit_integration_test.cpp`：只由完整无 Skip package 授权运行；再次校验 fresh package core 的规范路径/SHA-256/非生产 identity，以 `NoProxy` raw Qt HTTP/2 client 和 test-owned QProcess 验证协议 v2 non-admission 对账 fence、迟到 Exit 拒绝和正常退出。配置无 listener、无 TUN；失败清理先尝试已鉴权 Stop/Exit，最后才可 terminate/kill 精确 test-owned PID。它不是 GUI→Client E2E，只比较 WinINet 的 `ProxyEnable`、`ProxyServer`、`AutoConfigURL`、`ProxyOverride`、`AutoDetect` 五键。
- `go/cmd/nekobox_core/core_lifecycle_test.go`：覆盖失败 candidate、blocked Close、旧 reference、dial/stats/Stop 互斥、并发 Start、Exit STOPPED 前置与终态 `EXITING`；另覆盖 reconcile barrier 先挡迟到 Start/Exit、等待阻塞 Start/Stop、精确 active/failed-clean/blocked target、config hash 与 ordering watermark。`grpc_box_test.go` 覆盖 Exit/对账映射与非法 barrier；`grpc_exit_integration_test.go` 通过真实 localhost gRPC 验证 ACK 交付后 GracefulStop。`go/grpc_server/auth` 和 `grpc_identity_test.go` 覆盖 token + daemon UUID、协议 v2、one-shot shutdown controller、metadata 清除和握手回显。

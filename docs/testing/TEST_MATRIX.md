...
| CTest | 1/1 | `config_recovery_test` 覆盖内容寻址覆盖备份、隔离证据、删除前快照、幂等、篡改/越界拒绝、元数据原因合并和外部修改竞争；不代表 ProfileManager 事务或 ConfigBuilder/GUI 已有 C++ golden |
...
| L1/L3 | C++/Go/脚本自动测试 | CTest、自有 Go 模块与隔离导出 fixture | CTest 现有恢复基础设施 1 项，含删除前快照 helper，但尚无 ProfileManager 多文件事务 harness；两个 Go 模块的普通与 race 测试已有通过证据，含 nil-config/system-fallback/FullTest 窄回归；PowerShell/Python fixture 有独立证据，但 ConfigBuilder/import/runtime 仍缺 C++ golden | 阻断稳定版 |

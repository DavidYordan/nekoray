...
| CTest | 1/1 | `config_recovery_test` 覆盖内容寻址证据、三文件事务、回滚/锁/恢复方向、单文件 `VerifiedBefore`/`VerifiedAfter`/`Indeterminate` intent、终态退役、隐藏/意外事务条目、精确大小写身份和协议 staging；合法 terminal header + 损坏/空 entries 不阻断 startup 但 report 必须 `valid=false`，非法 terminal schema 仍阻断；不代表订阅/非空 group 或 ConfigBuilder 已有 C++ golden |
...
| L1/L3 | C++/Go/脚本自动测试 | CTest、自有 Go 模块与隔离导出 fixture | CTest 现有恢复/事务基础设施 1 项，含确定性提交、回滚、锁、pending、单文件 intent/退役，以及 startup 浅验 header 与 report 深验 entries/snapshot 的边界；尚无 ProfileManager/订阅完整 harness。两个 Go 模块的普通与 race 测试已有通过证据；PowerShell/Python fixture 有独立证据，但 ConfigBuilder/import/runtime 仍缺 C++ golden | 阻断稳定版 |

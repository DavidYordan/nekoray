...
| 生成配置双专用端口/front-proxy 回环运行 | 通过 | 提交 `55bb799`：隔离 profile/group/映射/route 经 GUI 审计导出后由 core 启动；A 经 group front proxy 返回 210，B 单跳返回 211，terminal 后的 `bypass` 不能移动 A，reject 不触达三个上游；停止 A 前置代理而保持 terminal 存活后 A 失败、B 与主/辅三个 listener 均存活 |
| CTest | 5/5 | 配置恢复、runtime transition、分享格式、辅助端口路由编译和 resolver policy 纯测试通过 |

提交 `a3dee71`、`9a328a5`、`55bb799` 已推送。A group front proxy 两跳与 B 单跳 HTTP profile 现已完成 ProfileManager/ConfigBuilder → 最终 JSON → core 启动 → 双端口请求的同一闭环；显式 chain profile 仍只完成生成与 schema 检查。本轮没有启动普通 GUI、真实 AnyTLS/Trojan 节点或 Windows TUN，也没有执行完整 package；真实节点 profile 与 Windows GUI 集成仍待验证。
...
- `test/test_auxiliary_route_runtime.ps1`：在隔离 appdata 中持久化主/辅 HTTP profiles、A group front proxy、映射和 route，经 GUI 审计导出后启动当前 core，以三个回环 HTTP proxy 桩验证两跳/单跳不同端口出口、terminal 后不跨线、reject 不触达上游，以及 A 前置代理失效时不绕过仍存活的 terminal 且不影响 B；主端口仅在该隔离配置中改为 `18119`，不改变默认 `2080`。它不代表显式 chain profile 或真实 AnyTLS/Trojan 节点。

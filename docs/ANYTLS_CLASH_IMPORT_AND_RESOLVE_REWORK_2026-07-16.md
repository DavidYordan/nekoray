...
- [x] 订阅层级 client/DoH/fallback 管理 UI 已在分组编辑面板提供，并支持分别批量应用 client、resolver，或一次性重置线路为继承默认值。
- [x] 单线路编辑 UI 已增加“继承订阅 / 覆盖订阅”状态。
- [x] 辅助线路端口已接入右键解析出站选择，并在配置生成时作为独立出站加入统计列表。
- [x] 辅助端口已支持基础设置中的可配置端口池和持久化运行意图；显式停止主代理会清空辅助端口，恢复主线路时会恢复仍有效的辅助端口。
- [x] 删除正在作为辅助端口运行的线路时会同步移除辅助状态并重载主配置；内部 TUN 正运行时该删除路径会被阻断。

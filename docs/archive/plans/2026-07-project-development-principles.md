...
> 状态：历史 Agent 工作计划，不是现行产品需求。
> 归档日期：2026-07-20
> 替代文档：[产品定义](../../PRODUCT.md)、[路线图](../../ROADMAP.md)、[开发工作流](../../development/WORKFLOW.md)。
> 注意：历史路径已经脱敏或可能失效，正文不代表当前需求或验收结果。
...
6. Naive 依赖外部核心，当前 RouteFluent/sing-box 主线不支持。新建、订阅导入、默认 UI、bean 类型和分享链接解析/导出均不得继续生成 Naive；历史 `type=naive` 配置按未知/不支持配置处理。
...
2. 构建脚本必须只处理本项目目录下的部署实例：`deployment/windows64`。不得关闭或修改生产实例，例如 `<installed-dir>`。
...
1. 本机可能同时运行旧版生产 Nekoray 的 TUN 模式。除非用户明确要求，不得停止 `<installed-dir>` 下的进程。
2. 本项目测试实例只允许操作 `<package-dir>` 下的进程。
...
13. 验证运行态连通性时使用 `tools/verify_runtime_connectivity.ps1` 采样主端口、辅助端口、Clash API、系统代理指向和 curl 结果；脚本默认只读，不得默认启动/停止项目或生产实例。
...
1. AnyTLS、订阅 DoH、链式前置代理、辅助端口等运行时问题，必须优先导出 profile 最终 core 配置进行审计，避免只看数据库字段或 UI 状态下结论。
2. 导出的最终 core 配置包含真实 server、password、DoH 等敏感信息，只能用于本地排障，不得提交到 git，也不得写入公开文档。
3. 涉及 core 配置格式的改动，必须至少用 `nekobox_core.exe check -c` 或等价方式验证生成配置。
4. 当前机器若运行另一套 TUN、透明代理或强制路由，本项目运行时 EOF/超时只能作为受干扰环境观测，不得直接定性为项目配置错误或节点错误；配置正确性以最终 core 配置审计和 `check -c` 为准，连通性以用户干净环境手测为准。

...
- `recovery/transactions/<UUID>/manifest.json`：多文件事务的操作名、状态、目标相对路径及 before/after SHA256。相邻 `before/`、`after/` 保存逐字节证据；只有清单以原子方式从暂存目录发布后，目标文件才可能开始变化。
...
- 单 profile/空 group 删除及 profile 跨组移动涉及的配置会在同一个事务中串行处理；进程内互斥与 `active.lock` 防止同机其它实例同时提交。正常状态为 `committed`、`rolled_back` 或尚未改目标的 `aborted`；活动锁、`prepared`、`rollback_failed`、缺失/损坏清单均会阻断另一实例加载。
...
若启动报告未完成事务，不要删除 `recovery/transactions/<UUID>` 或手工拼接唯一真实配置。先复制完整 `config/`，再核对 manifest、before/after SHA256 与每个目标当前内容；恢复 UI 尚未完成，应把该状态视为维护阻断。

...
显式删除单 profile 或空 group 现在也要求磁盘原文与加载快照一致，并先向 `recovery/deletions/` 写入内容寻址、回读校验的删除前快照；被 front proxy/chain 引用的 profile 拒绝删除。涉及的主配置、group order 与删除目标，以及 profile 跨组移动涉及的旧/新 group 和 profile，已接入 `recovery/transactions/` 事务清单：进程内和跨进程串行，先持久化 before/after，再逐项提交；普通失败会逆序回滚，无法确认的 `prepared`/`rollback_failed` 状态会禁止当前进程继续保存并阻断下次启动，不会自动猜测恢复方向。

旧的非空 group 删除会忽略子项失败并继续删 group，现仍整体拒绝；订阅清理/回滚会检查删除结果并保留失败对象，但订阅成功候选和非空 group 尚未接入事务层。
...
仍缺：可选择恢复来源和明确提交/回滚方向的 UI、订阅/非空 group 的事务接入、保存失败的强类型结果、显式悬空引用修复、备份保留/清理策略及真实进程中断/磁盘故障注入。事务层能检测崩溃留下的未完成状态，但尚不能自动恢复，因此不能称为完整可操作恢复体系。内容寻址证据可能持续增长，当前绝不自动删除。
...
残余风险在成功候选提交阶段：多个 `AddProfile`、group 保存与旧 profile 删除仍是独立文件操作，全局 ProfileManager mutation 也尚未全部收口到事务层。当前删除事务基础不能覆盖订阅更新；崩溃/磁盘故障仍可能留下新增、重复、残留节点或陈旧引用。
...
其它无侵入回归为：配置保留/隔离/事务阻断 9/9、OpenWrt helper Python 单测 19/19、本地 Mixed fixture 7/7；runtime connectivity 的 204 正例通过，错误期望 200 被正确拒绝。它们不覆盖真实 Windows TUN/WFP/退出/切线。

...
已完成单文件止损：`JsonStore::Save()` 使用禁止 direct-write fallback 的 `QSaveFile`，检查完整写入和 commit 后才更新内存快照；已有但无效的关键配置会保留并中止启动，未知/损坏 profile/group 原件保留且 ID 不复用；危险 legacy reorder 被 fail-closed 禁止。

仍缺：自动可验证备份、quarantine/UI 恢复、跨多个 profile/group 的事务、保存失败的强类型结果、悬空引用修复和故障注入测试。
...
停止、重启、崩溃或退出过去会静默清空辅助端口 map，加载/UI 刷新也会修剪未知/损坏映射。接管工作树已改为仅显式映射操作可修改：字段类型错误、非字符串、损坏或重复项会让既有主配置原件保持不变并中止启动；显式启停/删除在原子保存失败时回滚内存且不 reload。仍缺 quarantine/修复 UI 与这些路径的 C++ 单测。
...
其它 2026-07-20 无侵入回归为：配置保留 5/5、OpenWrt helper Python 单测 19/19、本地 Mixed fixture 7/7；runtime connectivity 的 204 正例通过，错误期望 200 被正确拒绝。它们不覆盖真实 Windows TUN/WFP/退出/切线。

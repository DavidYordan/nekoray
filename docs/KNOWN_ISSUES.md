...
2026-07-28 已把原生 `2080` 恢复为普通 route 序列，并移除 `mixed-in -> proxy` 的无条件终结规则；定向配置导出回归由旧构建 14/15 失败转为重建后 15/15 通过。随后专用端口改为 `sniff -> 仅该 inbound 的显式 reject/block -> 精确 chain terminal`；普通 direct、bypass、主线和其它 outbound 规则仍位于 terminal 之后。纯规则测试、脱敏 core schema 和双回环上游运行均通过；后者确认单上游失败不影响另一端口，但使用手写 fixture，仍缺真实 ConfigBuilder/ProfileManager/GUI 证据。

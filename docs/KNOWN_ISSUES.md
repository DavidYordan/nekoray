...
旧 GUI 把日志文本直接当 ready，而且只检查 stdout；Go `log.Printf` 默认写 stderr，所以真实 GUI 启动可能一直不发送 profile Start，表现为 `12080` 从未监听。现行实现同时读取 stdout/stderr，但日志只作为 `GetDaemonInfo` 探测提示；只有 UUID/协议版本精确回显才发布 ready，并对同一实例去重为一条有界重试链。该修复解释并关闭一个控制面阻断，仍没有真实 GUI/QProcess/HTTP2 集成证据，也不证明 profile 线路健康。
...
`DataStore::core_running` 与 `prepare_exit` 的跨线程访问现已改为 atomic bool；CoreProcess 也不再跨线程读取 `spmode_vpn` 来决定恢复，异常退出统一记录“empty core，不恢复 profile/system proxy/TUN”。退出/重启链会等待同一 UUID 的 Stop completion 或精确 stopped 对账：失败、blocked、superseded、字段不一致或 unknown 时绝不调用 core Exit/GUI quit，而是在 UI 线程撤销 `prepare_exit`/save freeze、恢复 hotkey/control 并保留 observed runtime。Start/Stop 的 HTTP/2 client 在 30 秒后 abort；随后使用的 5 秒 `ReconcileLifecycle` 不是只读状态查询，而是以更高序号取得同一 lifecycle mutex 并推进 ordering watermark 的屏障。两种时限都只界定 GUI 等待，不能取消 Go handler；对账自身超时后，目标操作或屏障仍可能稍后完成，GUI 必须继续保持 indeterminate。因此这不是端到端 deadline 或可恢复退出协议；Exit 仍未形成可靠 ACK + 精确 QProcess finished 闭环。`runtime_transition_test` 只覆盖模拟身份确认输入下的 queue/ready/stop 状态竞争，Go lifecycle 单测覆盖排序屏障和精确 target/hash/outcome；仍没有真实 QProcess/GUI crash→commit/退出及 HTTP/2 超时集成。该集成缺口仍是 process-local P0。
...
没有独立 WFP 过滤层时，简单删除 guard 或“先 check 再 Stop”都不能保证无直连。已选定的 P0 方向是持久 Windows Runtime Service、稳定 TUN/Mixed anchor、独立 persistent WFP kill-switch，以及候选 generation 预检和提交；提交前失败保留当前 generation，提交后故障进入阻断，禁止自动切回旧线路。只有用户明确选择、旧 generation 重新验证后才允许显式回滚。当前每次 core 启动已有独立 UUID，所有 RPC 在 handler 前验证精确身份，ready 也必须通过 UUID/协议握手；Start/Stop 响应不确定后会用同一 mutex 内的更高 command sequence 对账屏障，成功时可确认精确 active、failed-clean/fenced stopped 或原 runtime 仍 active。它解决 reused port/token 的跨 daemon 误投和一部分响应丢失，但 30 秒 client abort 仍不是可取消的端到端 deadline，对账自身也可能超时；此时 Go handler/屏障可能稍后完成而 GUI 仍必须保留 indeterminate。它也没有 Windows OS 事实源、Exit ACK 或精确 QProcess-finished 闭环，盲目重试仍禁止。
...
- core 的 `Start`/`Stop`/stats/dial/Exit 现已有 Go 侧 lifecycle mutex 与 generation-bound reference，GUI Start/Stop 也有单一 transition ticket。每代 daemon 的 UUID、握手和全 RPC identity header 封住跨重启误投；session-local command sequence 与对账 barrier 封住 handler 反超，并记录 target outcome/config hash。这仍只解决当前进程内直接竞态。Start/Stop 的 30 秒 client abort 与 5 秒对账等待都不是服务端可取消的端到端 deadline；系统仍没有 desired/observed/owner/health RuntimeStateMachine、持久 journal、Exit ACK/finished、独立 OS 事实源或 GUI/父进程死亡与真实 TUN/WFP 集成测试。

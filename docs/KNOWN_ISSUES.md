...
显式删除单 profile 或空 group 现在也要求磁盘原文与加载快照一致，并先向 `recovery/deletions/` 写入内容寻址、回读校验的删除前快照；被 front proxy/chain 引用的 profile 拒绝删除。旧的非空 group 删除会忽略子项失败并继续删 group，现已改为整体拒绝，直到多文件事务可用。订阅清理/回滚也会检查删除结果，失败对象保留并报告。

仍缺：可选择恢复来源的 UI、跨多个 profile/group/主配置的崩溃一致事务、保存失败的强类型结果、显式悬空引用修复、非空 group 安全事务删除、备份保留/清理策略及完整磁盘故障注入。内容寻址证据可能持续增长，当前绝不自动删除。
...
残余风险在成功候选提交阶段：多个 `AddProfile`、group 保存与旧 profile 删除仍是独立文件操作，全局 ProfileManager mutation 也尚未收口到单一串行事务。删除前快照和显式失败报告只能防止静默丢失，崩溃/磁盘故障仍可能留下新增、重复、残留节点或陈旧引用。

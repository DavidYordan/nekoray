...
当前 UI 已把 `spmode_vpn`（用户期望）与 `running_internal_tun`/外置 worker PID（worker 观测）分开显示，并记录成功或不确定 candidate 的 transition generation、daemon generation 与 config SHA-256；可明确暴露“requested; inactive”“ACTIVE; stop incomplete”和 indeterminate。受管 TUN observed 值来自最终通过校验的 candidate，而不是 RPC 完成后重新读取 mutable UI。由于 RPC 没有 expected generation，且 channel/token/port 在同一 GUI session 的 daemon restart 间复用，不确定 Start 无法证明实际接收者；GUI 在 commit 时以 `max(candidate, CurrentDaemonGeneration)` 作为保守本地清理上界。ack 后若 readiness 在最终 UI commit 前丢失，也显式进入 indeterminate。这个上界避免旧 crash 过早清除可能跨 restart 的请求，但可能让状态保留过久，直到上界 generation crash 或收到响应 daemon 已接受且按序完成的显式 Stop；后者仍不证明 daemon 身份。TrafficLooper binding 只在成功 UI commit 后发布。该观测仍不是 Windows 接口/路由/WFP 的事实源。core 崩溃只重启空控制 core，并保留期望值但不自动恢复 profile/TUN；这避免隐式启用，却会让实际 TUN/保护随 worker 消失，仍是 P0。

`DataStore::core_running` 与 `prepare_exit` 的跨线程访问现已改为 atomic bool；CoreProcess 也不再跨线程读取 `spmode_vpn` 来决定恢复，异常退出统一记录“empty core，不恢复 profile/system proxy/TUN”。退出/重启链会等待 Stop completion：Stop 失败或不确定时绝不调用 core Exit/GUI quit，而是在 UI 线程撤销 `prepare_exit`/save freeze、恢复 hotkey/control 并保留 observed runtime；只有收到响应 daemon 已接受且按序完成的 Stop 才继续。Start/Stop 的 HTTP/2 client 现会在 30 秒后 abort，从而界定 GUI 的等待，但超时仍按 indeterminate 处理：Go handler 未按该 context 中止，操作可能在服务端继续完成，且没有状态查询/跨 daemon 对账。因此这关闭了“无界 GUI 等待”“Stop 不确定却由父进程退出杀掉 core”和“旧 Start 在新 Stop 后抢锁启动”的窄路径，不是端到端 deadline 或可恢复退出协议。`runtime_transition_test` 已覆盖“先 ready、后 queue”和 `MarkProcessReady`/Queue 真并发恰好一次；Go lifecycle 单测覆盖新 Stop 拒绝迟到旧 Start，仍没有 QProcess/GUI crash→commit/退出及真实 HTTP/2 超时集成。该集成缺口仍是 process-local P0。
...
没有独立 WFP 过滤层时，简单删除 guard 或“先 check 再 Stop”都不能保证无直连。已选定的 P0 方向是持久 Windows Runtime Service、稳定 TUN/Mixed anchor、独立 persistent WFP kill-switch，以及候选 generation 预检和提交；提交前失败保留当前 generation，提交后故障进入阻断，禁止自动切回旧线路。只有用户明确选择、旧 generation 重新验证后才允许显式回滚。当前 GUI Start/Stop HTTP/2 调用只有 30 秒 client abort，没有端到端 deadline 或 indeterminate 后的状态查询/对账；token 是每个 GUI session 随机、在该会话内 daemon 重启沿用，`Start`/`Stop` 请求也不携带 expected daemon generation。session-local command sequence 能让同一 daemon 的显式 Stop 排在所有更旧命令之后，却不能绑定或查询 daemon generation。RPC 前后 readiness 检查只能缩小跨 daemon 窗口。请求已送达但响应丢失、client abort 或 daemon 变化时，GUI 现会保守记录 indeterminate candidate/profile/TUN 并等待显式 Stop；Go handler 不会因 client abort 保证中止，GUI 仍无法确认服务端实际状态。盲目超时重试可能重叠实际 generation，因此不能把进程内 mutex、command sequence 或 30 秒 abort 当作完整恢复协议。
...
- 首个 Windows-only CI 已建立，覆盖仓库卫生、固定子模块、受控 RouteFluent core 源构建、两个 Go 模块普通测试和 OpenWrt verifier 的无侵入安全契约；它尚不构建 GUI，也不验证 TUN/WFP、系统代理 broker 或切线。本地 CTest 最终为 2/2；其中 `runtime_transition` 覆盖 GUI transition 单一 owner、跨线程完成、迟到 completion fencing，以及 daemon generation/旧 crash timer/排队 profile 绑定与取消，仍不是 Runtime Service 或 Windows TUN/WFP 集成测试。直接运行测试必须把项目 MinGW `bin` 加入 `PATH`，否则缺失运行库可能以 `0xC0000135` 或 CTest 超时呈现。Go 除 nil-config、system-fallback 和 FullTest 窄回归外，新增 lifecycle candidate 发布/清理、blocked Close、跨代 reference/HTTP client、dial/stats 与 Stop 串行、并发 Start 和 Exit 前置状态测试；generation-bound HTTP transport 另覆盖禁用 keep-alive。core 的 count=20/race/vet 与 `grpc_server` 的普通/race/vet 均通过。仍没有 ConfigBuilder/import C++ golden。PowerShell 最终配置 guard 为 10/10；它不是 C++ golden 矩阵，整体覆盖仍有限。
...
- core 的 `Start`/`Stop`/stats/dial/Exit 现已有 Go 侧 lifecycle mutex 与 generation-bound reference，GUI Start/Stop 也有单一 transition ticket；session-local command sequence 另阻止同一 daemon 内新 Stop 被旧 Start 反超。这只解决当前进程内直接竞态。Start/Stop 的 30 秒 client abort 只界定 GUI 等待，不是服务端可取消的端到端 deadline。系统仍没有 desired/observed/owner/health 的 RuntimeStateMachine、持久 journal、RPC indeterminate 对账或独立 OS 事实源，现有单元/race 测试也未制造 GUI/父进程死亡与真实 TUN/WFP 生命周期竞争。
- 普通 GUI 使用 localhost、每个 GUI session 随机令牌的 gRPC，不能无令牌任意调用；同一会话内 daemon 重启会沿用该 token，它不是 expected-generation 绑定。但 Go `Start` 尚未重复 C++ ConfigBuilder 的 Mixed/TUN/系统代理/resolver 产品策略。`nekobox_core run/check` 又是显式高级 CLI，可直接读取 sing-box 配置并被测试工具使用。两者应准确视为 core 信任边界与纵深防御缺口，不应误写成普通 GUI 功能可任意绕过，也不能把 `check` 成功当成产品策略通过。
...
当前接管验证使用本轮重建的 `build-package-windows64/nekobox.exe`（SHA-256 `FDD4901C2836FACBCAAC6271B0D3463F8AB064458DD40BFACAB07C2B3F16BD15`）与 `nekobox_core.exe`（SHA-256 `6FE7236AF6866E6789CD41F3A0DA5C1F88ACCEC209E328E5449E4543AD0CE7A1`）。这些值仅是本地审计快照；`deployment/windows64/` 仍是 2026-07-18 的旧二进制，未做正式全量打包，不能作为交付证据。
...
已修复：打包不再自动关闭/强杀运行实例，`ReferenceDir` 默认空，空 fallback 路径异常已修复，并会按规范化直接路径拒绝把 `D:\Program Files\nekoray` 或其子目录作为 reference 输入，逐组件拒绝 `~` 与 ReparsePoint/junction。脚本尚未通过最终句柄消解 SUBST、设备路径等其它别名，操作者仍须独立确认 reference 根的最终目标；完整打包前仍需手动停止目标 deployment 实例，因为脚本会 fail-fast。

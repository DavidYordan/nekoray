...
namespace NekoGui_Runtime {
    ...
    struct DaemonProcessSnapshot {
        std::uint64_t generation = 0;
        std::string instanceId;
        std::int64_t processId = 0;

        [[nodiscard]] bool valid() const {
            return generation != 0 && !instanceId.empty() && processId > 0;
        }
    };

    struct DaemonProcessFinishedResult {
        DaemonProcessSnapshot process;
        int exitCode = -1;
        bool normalExit = false;
        bool valid = false;
    };

    // Records the immutable generation/UUID/PID assigned to one QProcess
    // launch and the exact QProcess::finished result for that launch. Waiting
    // happens through a standard condition variable so callers never invoke a
    // QProcess method from a foreign thread. The record is order independent:
    // finished may arrive before or after the RPC acknowledgement waiter.
    class DaemonProcessExitState {
    public:
        [[nodiscard]] bool MarkProcessStarted(const DaemonProcessSnapshot& process);

        [[nodiscard]] DaemonProcessFinishedResult MarkProcessFinished(
            int exitCode,
            bool normalExit);

        [[nodiscard]] DaemonProcessSnapshot CurrentProcess() const;

        [[nodiscard]] bool WaitForFinished(
            const DaemonProcessSnapshot& expected,
            std::chrono::milliseconds timeout,
            DaemonProcessFinishedResult* result = nullptr) const;

    private:
        mutable std::mutex mutex;
        mutable std::condition_variable finishedCondition;
        DaemonProcessSnapshot runningProcess;
        DaemonProcessFinishedResult lastFinished;
    };
